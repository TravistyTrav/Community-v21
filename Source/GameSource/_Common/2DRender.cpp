		...
		for( int i = nPos; i < nMax; i++ )
		{
			...
			{ 
				...
				if( dwStyle & ESSTY_BLOCK )
				{
					...
					dwBkgr = ~dwColor | 0xff000000; 
					RenderFillRect( CRect( x - m_ptOrigin.x, y - m_ptOrigin.y, x + size.cx - m_ptOrigin.x, y + size.cy - m_ptOrigin.y ), dwColor );
					m_pFont->DrawText( (FLOAT)( x ), (FLOAT)( y ), dwBkgr, temp, 0, wCodePage );
				}
...
void C2DRender::TextOut( int x,int y, int nValue, DWORD dwColor, DWORD dwShadowColor )
{
	...
	{
		TCHAR szString[ 32 ];
		FLIntToString( nValue, szString, _countof( szString ), 10 );
...
BOOL C2DRender::RenderTexture( CPoint pt, CTexture* pTexture, DWORD dwBlendFactorAlhpa, FLOAT fScaleX , FLOAT fScaleY )
{
	...
	pt += m_ptOrigin;
	CPoint ptCenter = pTexture->m_ptCenter;
	ptCenter.x	= (LONG)( (FLOAT)ptCenter.x * fScaleX );
	ptCenter.y	= (LONG)( (FLOAT)ptCenter.y * fScaleY );
	pt -= ptCenter;

	FLOAT left   = (FLOAT)( pt.x );
	FLOAT top    = (FLOAT)( pt.y );
	FLOAT right  = pt.x + ( fScaleX * (FLOAT)pTexture->m_size.cx );
	FLOAT bottom = pt.y + ( fScaleY * (FLOAT)pTexture->m_size.cy );
...
BOOL C2DRender::RenderTextureWithAlpha( const CPoint& rPt, CTexture* pTexture, CTexture* pAlphaTex, const CSize& kSize, DWORD dwColor )
{
	float fScaleX = 1.0f;
	float fScaleY = 1.0f;

	FLOAT left   = (FLOAT)rPt.x;//(FLOAT)( pt.x );
	FLOAT top    = (FLOAT)rPt.y; //FLOAT)( pt.y );
	...
	SetTextureVertex2( pVertices, left, top, pTexture->m_fuLT, pTexture->m_fvLT, dwColor );
	pVertices++;
	SetTextureVertex2( pVertices, right, top, pTexture->m_fuRT, pTexture->m_fvRT, dwColor);
	pVertices++;
	SetTextureVertex2( pVertices, left, bottom, pTexture->m_fuLB, pTexture->m_fvLB, dwColor);
	pVertices++;
	SetTextureVertex2( pVertices, right, bottom, pTexture->m_fuRB, pTexture->m_fvRB, dwColor);
	pVertices++;
...
{
	FLERROR_ASSERT_LOG_RETURN( pTexture, FALSE, _T("[2D Render] 텍스쳐 포인터가 유효하지 않습니다.") );

	ptStart += m_ptOrigin;
	CPoint ptCenter = pTexture->m_ptCenter;
	ptCenter.x	= static_cast<LONG>( ptCenter.x );
	ptCenter.y	= static_cast<LONG>( ptCenter.y );
	ptStart -= ptCenter;

	FLOAT left   = static_cast<float>( ptStart.x );
	FLOAT top    = static_cast<float>( ptStart.y );
	FLOAT right  = static_cast<float>( ptStart.x + ptOffset.x );
	FLOAT bottom = static_cast<float>( ptStart.y + ptOffset.y );
	//FLOAT right  = static_cast<float>( ptStart.x + 128 );
	//FLOAT bottom = static_cast<float>( ptStart.y + 128 );
	
	TEXTUREVERTEX vertex[ 4 ];
	TEXTUREVERTEX* pVertices = vertex; 
	
	SetTextureVertex( pVertices, left, top, l, t );
	pVertices++;
	SetTextureVertex( pVertices, right, top, r, t );
	pVertices++;
	SetTextureVertex( pVertices, left, bottom, l, b);
	pVertices++;
	SetTextureVertex( pVertices, right, bottom, r, b);
	pVertices++;
...
BOOL C2DRender::RenderTextureUV( CPoint pt, CPoint size, CTexture* pTexture, float l, float t, float r, float b )
{
	if( !pTexture )			//gmpbigsun
		return FALSE;
	...
	FLOAT left   = (FLOAT)( pt.x + ( (float)size.x * l ) );
	FLOAT top    = (FLOAT)( pt.y + ( (float)size.y * t ) );
	FLOAT right  = pt.x + ( fScaleX * ( (float)size.x * r ) );
	FLOAT bottom = pt.y + ( fScaleY * ( (float)size.y * b ) );
...
BOOL C2DRender::RenderTextureWithEffect1( CPoint pt, CTexture* pTexture, DWORD dwBlendFactorAlhpa, float fScaleX, float fScaleY )
{
	pt += m_ptOrigin;
	CPoint ptCenter = pTexture->m_ptCenter;
	ptCenter.x	= (LONG)( ptCenter.x * fScaleX );
...
BOOL C2DRender::RenderTextureWithEffect2( CPoint pt, CTexture* pTexture, CTexture* pEff, D3DXMATRIX* pMat, DWORD dwBlendFactorAlhpa )
{
	pt += m_ptOrigin;
	CPoint ptCenter = pTexture->m_ptCenter;
...
//	m_pd3dDevice->SetRenderState( D3DRS_CULLMODE, D3DCULL_NONE );	
	...
	m_pd3dDevice->SetTextureStageState( 1, D3DTSS_TEXTURETRANSFORMFLAGS, D3DTTFF_COUNT2 );
	m_pd3dDevice->SetTransform( D3DTS_TEXTURE1, pMat );
	m_pd3dDevice->SetTexture( 1, g_pReflectMap  ); //pEff->m_pTexture );
	m_pd3dDevice->SetTextureStageState( 1, D3DTSS_TEXCOORDINDEX, D3DTSS_TCI_CAMERASPACENORMAL );
...
//	HRESULT hr = pd3dDevice->CreateTexture( nWidth, nHeight, 1, Usage, Format, Pool, &m_pTexture, NULL );
	...
	HRESULT hr = D3DXCreateTexture( pd3dDevice, nWidth, nHeight, 1, Usage, Format, Pool, &m_pTexture );
...
//		}
#endif
		FLERROR_LOG( PROGRAM_NAME, _T( "텍스춰 생성 못함. Error = %d, cx = %d, cy = %d" ), hr, nWidth, nHeight );
		FLASSERT( 0 );
		return FALSE;
	}
...
BOOL CTexture::LoadTexture( LPDIRECT3DDEVICE9 pd3dDevice, LPCTSTR pFileName, D3DCOLOR d3dKeyColor, BOOL bMyLoader )
{
	...
	DWORD dwOffsetSrc = 0;
	FLStrcpy( szFileName, _countof( szFileName ), pFileName );
	...
	if( bMyLoader == FALSE ) // dds only
	{
		...
		if( hr != S_OK )
		{
			FLERROR_LOG( PROGRAM_NAME, _T( "%s read error" ), pFileName );
			return FALSE;		
		}
	...
	if( strstr( szFileName, ".bmp" ) )
	{
		...
		//#define BMP32BIT
			...
			{
			#ifndef _DEBUG
				FLERROR_LOG( PROGRAM_NAME, _T( "Lock 실패 : Error = %d" ), hr );
				FLASSERT( 0 );
			#endif
			}
		} 
			...
			for( int i = start.y; i < m_size.cy; i += size.cy )
			{
				for( int j = start.x; j < m_size.cx; j += size.cx, nCnt++ )
				{
					if( nCnt < nFrame )
					{
						...
						int x = j;
						int y = i;
						pTexture->m_fuLT = (FLOAT) x / (FLOAT)m_size.cx;
						pTexture->m_fvLT = (FLOAT) y / (FLOAT)m_size.cy;
						pTexture->m_fuRT = (FLOAT) ( x + pTexture->m_size.cx - 0 ) / (FLOAT)m_size.cx;
						pTexture->m_fvRT = (FLOAT) y / (FLOAT)m_size.cy;
						pTexture->m_fuLB = (FLOAT) x / (FLOAT)m_size.cx;
						pTexture->m_fvLB = (FLOAT) ( y + pTexture->m_size.cy - 0 ) / (FLOAT)m_size.cy;
						pTexture->m_fuRB = (FLOAT) ( x + pTexture->m_size.cx - 0 ) / (FLOAT)m_size.cx;
						pTexture->m_fvRB = (FLOAT) ( y + pTexture->m_size.cy - 0 ) / (FLOAT)m_size.cy;
						pTexture->m_pTexture = m_pTexture;
					}
				}
			}
...
#ifdef __CLIENT
MEMPOOLER_IMPLEMENT( CDamageNum, 32 );
...
//	else nAlpha = 255 - ( 255.0f / 80 ) * ( m_nFrame - 40 );
	...
	if( m_nAttribute < 4 ) { // 숫자인 경우
		// 한자리씩 차례대로 출력한다
		char strTemp[64];
		FLSPrintf( strTemp, _countof( strTemp ), "%d", m_nNumber );
		...
		for( int i = 0; i < nLength; i++ )
		{
			...
			{
				FLERROR_LOG( PROGRAM_NAME, _T( "DamageNum::Render : %s %d" ), strTemp, m_nAttribute );
			}
